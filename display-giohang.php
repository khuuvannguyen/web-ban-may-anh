<?php 
	require 'ketnoi.php';
	$sql="select * from giohang where ID='".$_SESSION['id']."' and MaDH is null";
	...
	if($number_of_results!=0){
		...
		if($page < 1 || $page > $number_of_pages || !is_numeric($page)){
			...
			$sql="select * from giohang where ID='".$_SESSION['id']."' and MaDH is null LIMIT ".$page_first_result.",".$result_per_page;
			...
			while($row=mysqli_fetch_array($resultG)){
				echo '<tr>';
					...
					$i+=1;
					$sql="select * from sanpham where MaSP='".$row['MaSP']."'";
					$result=mysqli_query($ketnoi,$sql);
					$sanpham=mysqli_fetch_array($result);
					echo '<td>'.$sanpham['TenSP'].'</td>';
					echo '<td style="text-align:center;">'.$row['SoLuong'].'</td>';
					echo '<td style="text-align:center;">'.number_format($sanpham['GiaTien'],'0',',','.').'</td>';
					$ThanhTien=$sanpham['GiaTien']*$row['SoLuong'];
					echo '<td style="text-align:center;">'.number_format($ThanhTien,'0',',','.').'</td>';
						...
						echo '<td style="text-align:center;">
								<a href="dathang.php?p='.$row['MaGH'].'&n='.$row['SoLuong'].'" class="btn btn-primary">Đặt hàng</a>
							</td>';
						echo '<td style="text-align: center;">
							<a href="xoa1.php?p='.$row['MaGH'].'" class="btn btn-danger">Xóa 1</a>
						</td>';
						echo '<td style="text-align: center;">
							<a href="xoahet.php?p='.$row['MaGH'].'" class="btn btn-danger">Xóa tất cả</a>
						</td>';
				echo '</tr>';
			}
			echo '</table>';
