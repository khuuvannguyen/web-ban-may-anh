<?php 
	require 'ketnoi.php';
	$sql="select * from donhang where ID='".$_SESSION['id']."'";
	...
	if($number_of_results!=0){
		...
		if($page < 1 || $page > $number_of_pages || !is_numeric($page)){
			...
			$sql="select * from donhang where ID='".$_SESSION['id']."' order by MaDH DESC LIMIT ".$page_first_result.",".$result_per_page;
			$result=mysqli_query($ketnoi,$sql);
			if(mysqli_num_rows($result)!=0){
				...
				while($row=mysqli_fetch_array($result)){
					echo '<tr>';
						...
						echo '<td class="text-center"><a href="hoadon.php?d='.$row['MaDH'].'" class="btn btn-primary" target="_blank">Xem</a></td>';
						if($row['XACNHAN']=='0'){
