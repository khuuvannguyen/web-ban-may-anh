<?php 
	...
	if($page < 1 || $page > $number_of_pages || !is_numeric($page)){
		...
		$sql="select * from donhang order by MaDH DESC LIMIT ".$page_first_result.",".$result_per_page;
		$result=mysqli_query($ketnoi,$sql);
		if(mysqli_num_rows($result)!=0){
			...
			while($row=mysqli_fetch_array($result)){
				echo '<tr>';
					echo '<td style="text-align:center;"><b>'.$row['MaDH'].'</b></td>';
					echo '<td style="text-align:center;">'.$row['NgayMua'].'</td>';
					echo '<td style="text-align:right;">'.number_format($row['TongTT'],0,',','.').'</td>';
					if($row['XACNHAN']=='0'){
					...
					echo '<td class="text-center"><a href="hoadon-nhanvien.php?d='.$row['MaDH'].'" class="btn btn-primary" target="_blank">Xem</a></td>';
					if($row['XACNHAN']=='0'){
						echo '<td class="text-center"><a href="xacnhan.php?d='.$row['MaDH'].'" class="btn btn-success">Xác nhận</a></td>';
						echo '<td class="text-center"><a href="huy.php?d='.$row['MaDH'].'" class="btn btn-danger">Hủy</a></td>';
					}else{
						echo '<td class="text-center"><a href="xacnhan.php?d='.$row['MaDH'].'" class="btn btn-success disabled">Xác nhận</a></td>';
						echo '<td class="text-center"><a href="huy.php?d='.$row['MaDH'].'" class="btn btn-danger disabled">Hủy</a></td>';
					}
