<?php 
	...
	if($page < 1 || $page > $number_of_pages || !is_numeric($page)){
		...
		$sql="select * from nhanvien LIMIT ".$page_first_result.",".$result_per_page;
		$result=mysqli_query($ketnoi,$sql);
		...
		while($row=mysqli_fetch_array($result)){
			echo '<tr>';
				if($row['TinhTrang']=="1"){
					...
					echo '<td style="text-align:center;"><a href="nhanvien-disabled.php?TenDN='.$row['TenDN'].'" class="btn btn-primary">Vô hiệu hóa</a></td>';
					echo '<td style="text-align:center;"><a href="nhanvien-undisabled.php?TenDN='.$row['TenDN'].'" class="btn btn-success disabled">Bỏ vô hiệu hóa</a></td>';
				}else{
					echo '<td style="background-color:#989898"><b>'.$row['TenDN'].'</b></td>';
					echo '<td style="background-color:#989898">'.$row['MatKhau'].'</td>';
					echo '<td style="text-align:center;background-color:#989898"><a href="nhanvien-disabled.php?TenDN='.$row['TenDN'].'" class="btn btn-primary disabled">Vô hiệu hóa</a></td>';
					echo '<td style="text-align:center;background-color:#989898"><a href="nhanvien-undisabled.php?TenDN='.$row['TenDN'].'" class="btn btn-success">Bỏ vô hiệu hóa</a></td>';
